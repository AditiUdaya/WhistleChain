...
// 4️⃣ Upload + Blockchain record + Notify receiver
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    // PQC Encryption (authenticated encapsulate)
    ...
    // Blockchain hash (of ciphertext)
    const hash = crypto.createHash("sha256")
      .update(Buffer.from(encrypted.ciphertext, "base64"))
      .digest("hex");

    // Record on blockchain
    const tx = await recordLeakHash("0x" + hash);
    ...
    const meta = {
      ...
      signature: Buffer.from(akem.signature).toString("base64"),
      senderSigPublicKey: userKeys[sender].sigPublicKey,
      hash,
    ...
    // ✅ NEW: Send PQC message to Bob with decrypt instructions
    const notificationText =
      `📁 New leak uploaded by ${sender}!\n\n` +
      `To decrypt, run:\nnode decrypt.js ${encFileName} ${receiver} http://localhost:3000`;

    const notifEnc = encryptAES(aesKey, notificationText);
    ...
    messages.push({
      ...
      signature: Buffer.from(akem.signature).toString("base64"),
      iv: notifEnc.iv,
      ciphertext: notifEnc.ciphertext,
